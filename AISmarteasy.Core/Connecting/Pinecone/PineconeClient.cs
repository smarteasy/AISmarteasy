...
public sealed class PineconeClient : IPineconeClient
{
    public PineconeClient(string environment, string apiKey, ILoggerFactory? loggerFactory = null)
    {
        ...
        _httpClient = new HttpClient(Handling.NonDisposableHttpClientHandler.Instance, disposeHandler: false);
    }
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var responseContent = await HttpContentExtensions.ReadAsStringWithExceptionMappingAsync(response.Content).ConfigureAwait(false);
