...
public class PineconeMemoryStore : IPineconeMemoryStore
{
    ...
    {
        await foreach (var index in _pineconeClient.ListIndexesAsync(cancellationToken).ConfigureAwait(false))
        {
            yield return index ?? "";
        }
    }
    ...
    public async Task DeleteCollectionAsync(string collectionName, CancellationToken cancellationToken = default)
    {
        if (await DoesCollectionExistAsync(collectionName, cancellationToken).ConfigureAwait(false))
        {
            await _pineconeClient.DeleteIndexAsync(collectionName, cancellationToken).ConfigureAwait(false);
        }
    }
    ...
    {
        (PineconeDocument vectorData, OperationTypeKind operationType) = await EvaluateAndUpdateMemoryRecordAsync(indexName, record, indexNamespace, cancellationToken).ConfigureAwait(false);

        Task request = operationType switch
        {
            OperationTypeKind.Upsert => _pineconeClient.UpsertAsync(indexName, new[] { vectorData }, indexNamespace, cancellationToken),
            OperationTypeKind.Update => _pineconeClient.UpdateAsync(indexName, vectorData, indexNamespace, cancellationToken),
        ...
        [EnumeratorCancellation] CancellationToken cancellationToken = default)
    {
        ...
        {
            (PineconeDocument document, OperationTypeKind operationType) = await EvaluateAndUpdateMemoryRecordAsync(
                indexName,
                record,
                indexNamespace,
        ...
        {
            tasks.Add(_pineconeClient.UpsertAsync(indexName, upsertDocuments, indexNamespace, cancellationToken));
        }

        if (updateDocuments.Count > 0)
        {
            IEnumerable<Task> updates = updateDocuments.Select(async d
                => await _pineconeClient.UpdateAsync(indexName, d, indexNamespace, cancellationToken).ConfigureAwait(false));
    ...
    {
        return await GetFromNamespaceAsync(collectionName, string.Empty, key, withEmbedding, cancellationToken).ConfigureAwait(false);
    }
        ...
        CancellationToken cancellationToken = default)
    {
        try
        {
            await foreach (PineconeDocument? record in _pineconeClient.FetchVectorsAsync(
                               indexName,
                               new[] { key },
                               indexNamespace,
                               withEmbedding,
    ...
    public async IAsyncEnumerable<MemoryRecord> GetBatchAsync(
        ...
        [EnumeratorCancellation] CancellationToken cancellationToken = default)
    {
        await foreach (MemoryRecord? record in GetBatchFromNamespaceAsync(collectionName, string.Empty, keys, withEmbeddings, cancellationToken).ConfigureAwait(false))
        {
            yield return record;
        }
    }
        ...
        [EnumeratorCancellation] CancellationToken cancellationToken = default)
    {
        ...
        {
            MemoryRecord? record = await GetFromNamespaceAsync(indexName, indexNamespace, key, withEmbeddings, cancellationToken).ConfigureAwait(false);
    ...
    public async IAsyncEnumerable<MemoryRecord?> GetWithDocumentIdAsync(string indexName,
        ...
        [EnumeratorCancellation] CancellationToken cancellationToken = default)
    {
        await foreach (MemoryRecord? record in GetWithDocumentIdBatchAsync(indexName, new[] { documentId }, limit, indexNamespace, withEmbedding, cancellationToken).ConfigureAwait(false))
        {
            yield return record;
        }
    }
        ...
        [EnumeratorCancellation] CancellationToken cancellationToken = default)
    {
        foreach (IAsyncEnumerable<MemoryRecord?>? records
                 in documentIds.Select(
                     documentId => GetWithDocumentIdAsync(indexName, documentId, limit, indexNamespace, withEmbeddings, cancellationToken)))
        {
        ...
        [EnumeratorCancellation] CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            vectorDataList = await _pineconeClient
    ...
    public async Task RemoveFromNamespaceAsync(string indexName, string indexNamespace, string key, CancellationToken cancellationToken = default)
    {
        try
        {
            await _pineconeClient.DeleteAsync(indexName, new[]
                {
                    key
                },
                indexNamespace,
    ...
    {
        await Task.WhenAll(keys.Select(async k => await RemoveFromNamespaceAsync(indexName, indexNamespace, k, cancellationToken).ConfigureAwait(false))).ConfigureAwait(false);
    }
        ...
        CancellationToken cancellationToken = default)
    {
        try
        {
            await _pineconeClient.DeleteAsync(
                indexName,
                default,
                indexNamespace,
                filter,
    ...
    public async Task RemoveWithDocumentIdAsync(string indexName, string documentId, string indexNamespace, CancellationToken cancellationToken = default)
    {
        try
        {
            await _pineconeClient.DeleteAsync(indexName, null, indexNamespace, new Dictionary<string, object>()
            {
    ...
    public IAsyncEnumerable<(MemoryRecord, double)> GetNearestMatchesAsync(string collectionName, ReadOnlyMemory<float> embedding,
        int limit, double minRelevanceScore = 0, bool withEmbeddings = false, CancellationToken cancellationToken = default)
    {
        return GetNearestMatchesFromNamespaceAsync(collectionName, string.Empty, embedding, limit,
            minRelevanceScore, withEmbeddings, cancellationToken);
    }
    ...
    public async Task<(MemoryRecord, double)?> GetNearestMatchAsync(
        ...
        CancellationToken cancellationToken = default)
    {
        return await GetNearestMatchFromNamespaceAsync(
            collectionName,
            string.Empty,
            embedding,
            minRelevanceScore,
    ...
    public async Task<(MemoryRecord, double)?> GetNearestMatchFromNamespaceAsync(
        ...
        CancellationToken cancellationToken = default)
    {
        IAsyncEnumerable<(MemoryRecord, double)> results = GetNearestMatchesFromNamespaceAsync(
            indexName,
            indexNamespace,
            embedding,
    ...
    public async IAsyncEnumerable<(MemoryRecord, double)> GetNearestMatchesWithFilterAsync(
        ...
        [EnumeratorCancellation] CancellationToken cancellationToken = default)
    {
        IAsyncEnumerable<(PineconeDocument, double)> results = _pineconeClient.GetMostRelevantAsync(
            indexName,
            embedding,
            minRelevanceScore,
            limit,
    ...
    {
        await _pineconeClient.DeleteAsync(indexName, default, indexNamespace, null, true, cancellationToken).ConfigureAwait(false);
    }
    ...
    {
        IndexStats? indexStats = await _pineconeClient.DescribeIndexStatsAsync(indexName, default, cancellationToken).ConfigureAwait(false);
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        PineconeDocument? existingRecord = await _pineconeClient.FetchVectorsAsync(indexName, new[] { key }, indexNamespace, false, cancellationToken)
